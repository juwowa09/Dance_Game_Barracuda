...
public class BarracudaRunner : MonoBehaviour
{
	...
	private void UpdateAvatar()
	{
		...
		{
			if (inputs[inputName3] != null) inputs[inputName3].Dispose();
			if (inputs[inputName2] != null) inputs[inputName2].Dispose();
			if (inputs[inputName1] != null) inputs[inputName1].Dispose();
	...
	private void PredictPose()
	{
		...
		if (useLowPassFilter)
		{
			foreach (JointPoint jp in avatar.jointPoints)
			{
				...
				Message += point+"|"+jp.pos3D.x + "|" + jp.pos3D.y + "|" + jp.pos3D.z + "\n";
				point++;
			}
		}
